...
- открытые файлы больше не задерживают друг друга: за один заход разбирается один файл, активный — первым, а очень большой файл обрабатывается частями, не подвешивая редактор;
- Completion, Hover и Signature Help больше не отвечают по устаревшей модели файла — раньше сразу после ввода текста подсказка могла относиться к другому символу или приходить пустой;
- импортированные файлы читаются и разбираются в отдельном потоке и хранят только то, что видно снаружи: индексация проекта не подтормаживает набор текста, а память расходуется примерно в пять раз экономнее;
- семантическая подсветка обновляется после загрузки импортированных модулей, а не остаётся устаревшей до следующей правки;
- Problems публикуются двумя волнами — сначала ошибки самого файла, затем межфайловые — без мерцания и без смешивания версий;
- неоднозначный `Import` больше не пропускается молча: один из кандидатов выбирается предсказуемо, а предупреждение остаётся в Problems;
- файл из одного очень длинного выражения больше не остаётся без подсказок и Problems из-за ошибки разбора.
