...
- разбор больше не выносится в отдельный поток: замеры показали, что передача дерева разбора обратно стоит дороже самого разбора, поэтому вместо этого ограничена длина порции очереди — за один проход разбирается один документ, активный первым, и открытые вкладки больше не задерживают ответы;
