...
- правки в больших файлах обрабатываются точечно заметно чаще — пересчёт идёт по изменённой строке, поэтому под него попадает обычный набор текста, а не только правка внутри одного слова;
