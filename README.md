...
## Что изменилось в 1.1.10

- переключение вкладок больше не блокирует редактор: раньше каждый файл, через который вы прошли, обрабатывался целиком прямо в момент переключения — на двенадцати файлах по 200 КБ это больше секунды без подсказок и Problems;
- разбор, ставший ненужным, пропускается при запуске: закрытый или уже изменённый файл не разбирается в прежней версии;
- пересчёт токенов при правке стал точечным заметно чаще — по изменённой строке, а не по одному токену, поэтому под него попадает обычный набор текста; разбор и Problems при этом считаются для файла целиком;
- изменённый импортированный файл больше не может быть принят за неизменённый: сравнивается содержимое, а не дата изменения, которую сохраняют системы контроля версий;
- сводки импортированных файлов сохраняются между запусками — повторное открытие проекта не сканирует его заново;
- параметр внешней `Macro` больше не помечается ошибкой при использовании во вложенной в неё `Macro`;
- все 239 стандартных процедур получили описание по существу вместо общей заглушки, исправлены возвращаемые типы и сигнатуры части из них, а возвращаемый тип теперь виден в подсказке параметров;
- методы и свойства стандартных классов получили описания вместо заглушек, а имена свойств RSD исправлены на документированные: автодополнение предлагало несуществующие `CommandText`, `FieldCount`, `RecordCount` вместо настоящих `CmdText`, `FldCount`, `RecCount`;
- автодополнение после точки показывает унаследованные свойства и методы — и для `Class (Base)` в вашем коде, и для стандартных классов;
- появились подсказки по прикладным модулям платформы (`CommonInter`, `BankInter`, `PaymInter`, `RsbFormsInter` и другие) — 277 классов, 1555 процедур и константы модулей: предлагаются только там, где модуль импортирован, а наследование доходит до стандартных классов;
- тип переменной выводится из объявленного типа результата процедуры: `rs = execSQLselect(...)` даёт подсказку по `rs.MoveNext()`, в том числе когда переменная объявлена без `Var`.
...
```text
code --install-extension rsl-plus-1.1.10.vsix
```
