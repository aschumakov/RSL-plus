...
test("ключом модуля становится то, что пишут в Import", () => {
    /*
     * Прежде кириллическому названию ключом доставалось имя корневой
     * страницы — `calendar` вместо `Календарь`. Каталог тогда не знает
     * ни одного написания, которое встречается в коде: `Import Календарь`
     * — 192 файла настоящего проекта, `Import calendar` — ни одного, — и
     * панель зависимостей называет такой модуль «не найден».
     */
    const known = new Map();
    assert.strictEqual(
