...
test("лексер на вложенных скобках растёт линейно", () => {
    ...
    /*
     * Замеры чередуются, и от каждого берётся лучшее время.
     *
     * Две серии подряд попадают в разную загрузку машины: на занятом
     * компьютере проверка падала не из-за лексера. Чередование и минимум
     * убирают именно это, а не ту разницу, ради которой проверка написана.
     */
    let indexed = Number.POSITIVE_INFINITY;
    let plain = Number.POSITIVE_INFINITY;

    for (let run = 0; run < 7; run++) {
        indexed = Math.min(indexed, milliseconds(() => lexRsl(withIndex), 1));
        plain = Math.min(
            plain,
            milliseconds(() => lexRsl(withoutIndex), 1)
        );
    }
    ...
    /*
     * Сравниваются два текста одного размера: так из замера уходит и разница
     * в объёме памяти, и разница в числе токенов. Обход назад делал текст с
     * `)[` в двадцать пять раз дороже, поэтому порог здесь с запасом: он
     * ловит возврат квадратичного роста и не ловит шум машины.
     */
    assert.ok(
        ratio < 4,
