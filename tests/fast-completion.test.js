...
test("тип не переносится между Macro", async () => {
    ...
    /*
     * И общих имён тоже: после точки их не бывает ни при каком
     * незнании о получателе. Прежде здесь ожидался обычный список — это
     * и приводило к `MsgBox` после точки.
     */
    assert.ok(
        !labels.includes("MsgBox"),
        `После точки общих имён быть не должно: ${labels.join(", ")}`
    );
});
