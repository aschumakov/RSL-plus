...
test("слово Import опознаётся директивой везде, где стоит", () => {
     ...
     * обязаны видеть тоже.
     */
    const answers = extractAll("Macro First()\nEnd;\n. import alpha;" + TAIL);

    for (const who of [
        "синтаксическое дерево",
        "сканер ссылок",
        "модель Organize Imports"
    ]) {
        assert.deepStrictEqual(
            answers[who],
            ["alpha"],
            who + " обязан увидеть директиву не в начале предложения"
        );
    }

    /*
     * Двое строже: они следят за границами предложений и такую директиву
     * пропускают.
     *
     * Различие измерено и оставлено намеренно. Оно задевает 4 файла из 6166 —
     * те самые, где предыдущее предложение закрыто точкой вместо точки с
     * запятой. Свести всех к одному правилу нельзя без потери: ослабить
     * строгих — и `Var import = 1;` станет директивой у всех; ужесточить
     * остальных — и эти 4 файла лишатся своих зависимостей везде, а не только
     * в компактной сводке.
     *
     * Цена нынешнего положения: у этих 4 файлов компактная сводка и быстрый
     * индекс не знают их Import. Проверка держит различие на виду, чтобы его
     * меняли осознанно, а не «для порядка».
     */
    for (const who of ["компактные объявления", "быстрый индекс"]) {
        assert.deepStrictEqual(
            answers[who],
            [],
            who + ": различие обязано остаться заметным"
        );
    }
});
