...
async function run() {
    ...
    await test("без модели Completion отдаёт состав из быстрого снимка", async () => {
        ...
        assert.deepStrictEqual(
            labels,
            ["Doc", "Helper", "Test"],
            "классы и макропроцедуры обязаны быть доступны сразу"
        );

        /*
         * А метод класса — нет: самостоятельным именем он не вызывается, только
         * через объект. Внутри своего класса он, наоборот, обязан предлагаться.
         */
        assert.ok(
            buildRslFastCompletions(snapshot, source.indexOf("Macro Save"))
                .some(item => item.label === "Save"),
            "внутри своего класса метод обязан предлагаться"
        );
