...
export class RslProjectIndexView {
     ...
     *
     * Имя говорит именно то, что метод делает: это КАНДИДАТЫ, а не ссылки.
     ...
     *
     * Имя `referencesOf(SymbolRef)` оставлено свободным: когда постоянные
     * записи о ссылках спрячутся за этот вход, оно должно достаться настоящим
     * точным ссылкам, а не списку файлов.
     */
    async referenceCandidateUris(
        declarationUri: string,
