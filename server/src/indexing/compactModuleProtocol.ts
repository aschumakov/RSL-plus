...
export interface ICompactModuleIndexed
     ...
     *
     * Отсечка для поиска ссылок: файл, в наборе которого искомого имени
     * нет, читать незачем. Считаются тем же чтением, что и объявления;
     * прежде их собирал ReferenceIndex, открывая тот же файл во второй
     * раз уже на основном потоке.
     *
     * Отсутствуют у ответа, поднятого из дискового кэша: там их не
     * хранят, чтобы не держать на диске второй экземпляр того, что уже
     * лежит в хранилище ссылок.
     */
