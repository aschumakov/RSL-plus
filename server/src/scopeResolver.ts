...
export class RslScopeResolver {
     ...
     */
    /**
     * Индекс проекта, на котором работает резолвер.
     *
     * Нужен слою типов: он обязан смотреть на тот же индекс, иначе ответы
     * разойдутся. Отдаётся только для чтения — менять его снаружи нельзя.
     */
    ...
    getCompletions(
        ...
        if (dotIndex >= 0) {
            ...
            /*
             * Точка есть, а простого получателя перед ней нет: значит слева
             * цепочка вроде `GetRecordset().` или `cmd.Execute().`. Разобрать
             * её здесь нечем — тип результата вызова знает слой типов, а он
             * стоит выше резолвера, — но провалиться в общий список нельзя:
             * после точки общих имён не бывает. Пустой ответ отдаст
             * вызывающему право спросить состав по цепочке.
             */
            return [];
        }
