 ...
 * MAX_SHIFTED_TOKEN_FRACTION в incrementalLex.ts.
 *
 * Столбец «принят» показывает решение пути, а не вывод замера: отсечка стоит
 * по доле сдвигаемых токенов, и таблица нужна именно для того, чтобы увидеть,
 * совпадает ли она с реальной границей выгоды на конкретной машине. Строка,
 * где путь принят, но время близко к полному лексированию, означает, что
 * порог пора опускать — на медленной машине такая правка уже в минусе.
 */
