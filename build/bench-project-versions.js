 ...
 * загрузка импортов выключена. Что из этого следует:
 *
 *   достоверно  — lex, разбор и модель; проверки, зависящие только от текста,
 *                 и лента кэша `text`: они смотрят на сам файл и ни на что
 *                 больше;
 *   отличается  — и локальные, и межфайловые Problems. От импортов зависит не
 *                 только межфайловая фаза: в локальной это constantAssignment,
 *                 localVisibility, scalarMembers, coreDialect и
 *                 undeclaredAssignments. Без `lib.mac` с `Var Shared` строка
 *                 `Shared = 1` даёт «переменная не объявлена», а с ним не даёт.
 *                 Поэтому находок в изоляции может оказаться и больше, и
 *                 меньше, чем в проекте;
 *   занижено    — цена ленты `imports` и цена разрешения имён: разрешать почти
 *                 нечего. `imports 200/200` означает «при неизменном окружении
 *                 запись переиспользуется», а не «столько стоит настоящий
 *                 Import-контекст».
 *
 * Сравнение версий между собой на этом стенде корректно: обе меряются в
 * одинаковых условиях. Некорректно принимать здешние числа за ощущения от
 * работы с настоящим открытым проектом. Режим с заранее собранным каталогом и
 * подгруженными зависимостями — задача следующей версии.
 *
...
function printReport(report) {
    ...
    console.log(
        "  (зависимостей проекта в индексе нет: и локальные, и межфайловые\n" +
        "   Problems могут отличаться от настоящего проекта, а цена ленты\n" +
        "   imports и разрешения имён занижена; см. шапку стенда)"
    );
